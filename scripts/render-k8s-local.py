...
parser.add_argument("--contact-image", default="arkadiusz-kamrowski-contact:local")
...
if ":" not in args.image or ":" not in args.contact_image:
    raise SystemExit("--image and --contact-image must include a tag")

text = (ROOT / "k8s/local.yaml.tpl").read_text(encoding="utf-8")
text = text.replace("{{IMAGE}}", args.image).replace("{{CONTACT_IMAGE}}", args.contact_image).replace("{{NAMESPACE}}", args.namespace)
